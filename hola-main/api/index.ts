...
function getPkce(req: Request): { state: string; codeVerifier: string } | null {
  const raw = getCookie(req, "hola_pkce")
  if (!raw) return null
...
async function handleCallback(req: Request, url: URL): Promise<Response> {
  ...
  // Redirect back to the CCTI frontend if redirect_uri was provided, otherwise /profile
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (pathname === "/login") return handleLogin(url)
  ...
  // CCTI API routes
  const cctiRes = await cctiHandler(req)
  if (cctiRes) return cctiRes
